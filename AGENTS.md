...
- 局部作者类型统一为 Css，内置 ThemeCss 继承 Css，用户可选择任一层扩展。useStyleRuntime 只接受可选的 context/theme/cssType 选项对象，不保留旧位置参数或 StyleBuilder 别名。框架接入桥在 core/style-scope，业务调用从适配器根入口导入。
