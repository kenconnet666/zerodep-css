...
P1—P6 已完成约定的生产范围，包含作者语义、薄 core、原生响应式与主题生命周期、性能优化、Nuxt/Kit 适配及最终 API/目录审核。[2c51b0f 的完整远程 CI](https://github.com/kenconnet666/zerodep-css/actions/runs/35946894347)九个 job 全部成功。此后的文档与研究样本清理不改变已验收的产品代码；后续产品修改仍须按对应 SHA 核对结果。[路线图](roadmap.md)记录下一阶段候选。
...
此后新增的动态 raw/token 绑定优化本地通过根 check/build、219 项单元、三语言类型负例、运行时及 Vue/Svelte 浏览器回归、体积门禁；其计算与浏览器探针、未采用候选见[性能对照](performance.md)。Nuxt/Kit 独立部署未在这次局部改动后本地重跑，但 [8226e53 对应的九项远程 CI](https://github.com/kenconnet666/zerodep-css/actions/runs/35952156619)全部通过，包含 Nuxt/Kit、三浏览器及独立消费。后续产品改动仍须核对自己的验收结果。
...
旧性能脚本和示例已从当前工作树清理，`.research/README.md` 提供历史源码位置；当前仅保留运行时主线的原始性能样本与锁文件。成功测试自动清理自己创建的临时应用，失败诊断留在忽略的 test-results。旧机器上被自动审批阻止删除的临时目录不在 Git，也无需换机复制。
