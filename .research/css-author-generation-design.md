...
第一阶段已实现可重复运行的生成脚本，先输出 **10 条代表性属性链、680 个关键字**；已确定的最终范围仍是 **502 个标准与独有 SVG 属性**。包体积不是筛选关键字的理由，优先保证写法自然。每个属性是一条链，常见用法直接产出声明字符串：
...
系统关键字、`px(number)` 和 `raw(value)` 是同一属性链上的三种输入。`raw('xx')` 是逃生舱：直接拼成 `width:xx;`，不解析、不校验、不补单位，浏览器决定该声明是否生效。第一阶段直接沿用属性值类型提供已知值提示，任意普通字符串始终可用。主题关键字通过继承属性链添加，例如 `s.width._md`，不需要另一套关键字 API。
...
2. **注释元数据：第一阶段使用 `csstype` 自带的初始值和 MDN 链接注释，并在 `scripts/css-author-pilot.json` 写入 10 条简短中文用途说明。** 扩展全量时可加入固定版本的 [MDN CSS 数据](https://github.com/mdn/data/blob/main/css/properties.json)交叉检查语法、初始值、继承性和状态；[维护说明](https://github.com/mdn/data/blob/main/docs/updating_css_json.md)说明这些字段来自规范。MDN 数据不另行决定快捷关键字。
...
第一阶段生成文件在 `core/src/generated/author.ts`，手写的 `author.ts` 只转发导出；全量增长后可按属性名首字母分组，避免把数万行堆在一个文件。每个属性链至少有简短 JSDoc：CSS 原名、可取得的初始值、MDN 链接。扩展时可给更多高频属性补一句经过审核的中文用途说明；不自动翻译 MDN 文本，也不复制兼容性表格。关键字本身通常不逐个注释，以免生成文件膨胀；含义容易混淆的值可在手工映射中添加短注释。

当前职责边界：`scripts/generate-css-author.mjs` 读取固定依赖并生成文件，`scripts/css-author-pilot.json` 保存首批属性、少量中文说明与简写 `px()` 参数上限，`core/src/generated/` 只放生成结果。输出头部标明来源版本和“请勿手改”。
...
第一阶段只实现核心属性链生成及类型检查，未实现类型化上下文注入和规则宿主归属。后续先验证这两处适配器边界，再展开 502 属性生成；CSS 变量编译优化继续只记录、不实施。
