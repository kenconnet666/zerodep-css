...
async function document(filePath) {
  ...
  return {
    ...
    kind: extension === '.svelte' ? 'svelte' : extension === '.vue' ? 'vue' : 'typescript',
    languageId:
      extension === '.svelte'
        ? 'svelte'
        : extension === '.vue'
...
async function start(kind) {
  const tsService = kind === 'vue' ? await service('typescript') : undefined;
  ...
  // Vue 3 的混合模式将 TS 语义请求交给安装了 Vue 插件的 tsserver。
  // 无论成功失败都响应 id，避免 Vue 服务保留永远不完成的请求。
  let forwardingError;
  const forwardTsRequests = async (requests) => {
    const batch = typeof requests[0] === 'number' ? [requests] : requests;
    for (const [id, command, payload] of batch) {
      let body = null;
      try {
        if (!tsService) throw new Error('TypeScript service is unavailable for Vue.');
        const response = await tsService.request('workspace/executeCommand', {
          command: 'typescript.tsserverRequest',
          arguments: [command, payload, { executionTarget: 0 }],
        });
        if (!response?.success) throw new Error('Vue TS request failed: ' + command);
        body = response.body ?? null;
      } catch (error) {
        forwardingError = error;
        process.stderr.write(String(error) + '\n');
      }
      await connection.sendNotification('tsserver/response', [id, body]);
    }
  };
  connection.onNotification('tsserver/request', forwardTsRequests);
  connection.onRequest('tsserver/request', async (requests) => {
    await forwardTsRequests(requests);
    return null;
  });
  connection.listen();
  ...
  try {
    const initialized = await request('initialize', {
      ...
      clientInfo: { name: 'Codex zerodep-css language bridge', version: '1' },
      rootUri: pathToFileURL(root).href,
      workspaceFolders: [{ name: 'zerodep-css', uri: pathToFileURL(root).href }],
      capabilities: {
    ...
    const relevant = /\.(?:[cm]?[jt]sx?|svelte|vue|json|yaml)$/u;
...
const server = new McpServer({ name: 'zerodep-language-services', version: '1.0.0' });
      ...
      try {
        ...
        for (let attempt = 0; ; attempt++) {
          try {
            ...
            // Vue 3 混合模式的 TS/模板语义由带 Vue 插件的 tsserver 提供。
            // Vue server 仍接收同一文档，用于模板专属能力；不能把其空诊断当语义成功。
            const semantic = doc.kind === 'vue' ? await service('typescript') : language;
            const run = () => language.run(doc, (version) => read(args, doc, semantic, version));
            const result = doc.kind === 'vue' ? await semantic.run(doc, run) : await run();
            ...
            services.delete(doc.kind);
            const child = activeChildren.get(doc.kind);
            if (child) stop(child);
          }
        }
      } catch (error) {
...
tool(
  'hover',
  'Get semantic TypeScript/Vue/Svelte type information. Input and output positions are 1-based.',
  location,
...
tool(
  'diagnostics',
  'Get completed diagnostics for one file. Uses Svelte pull diagnostics and synchronous TypeScript/Vue semantic/syntax requests; a timeout is an error, never an empty success. Full-project checks remain separate.',
  file,
