...
自动绑定迁移已开始：没有旧 `bx` 导入时，编译插件可将原生模板直接 `css(s => { s.padding.px(8, gap); })` 中的动态单位值编译为原生 style 绑定。当前仅提升无控制流、无未知调用的根声明；派生类、脚本快照、组件透传和复杂表达式保留运行时行为。完整迁移目标见[生产化计划](../.design/production-plan.md)。
