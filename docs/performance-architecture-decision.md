...
日期：2026-09-24。状态：原阶段的通用快路径取舍。本记录依据当时 `codex/runtime-first` 的锁定版本和样本；后来用户明确要求先处理响应式变量与模板提升，现已实现可证明的**单条声明**固定规则提升，范围见[编译器](compiler.md)、结果见[性能记录](performance.md)。下文不引入任意回调通用执行计划、共享节点池或函数身份缓存的判断仍保留为历史依据。完整运行时 CSS、普通同步 JavaScript 回调、框架原生响应式、SSR 请求宿主和逐记录事务仍是产品合同。

## 结论

**当时不引入通用的“固定 CSS 结构＋动态参数”执行计划，也不改用共享 style 节点或按回调身份缓存。** 当时保留静态准备、原位置动态绑定和完整运行时回退。动态 raw/token 的双重值判定已在 `8226e53` 消除；该结论针对任意回调的通用快路径，不否定后来经真实 Vue/Svelte 组件验证的单条声明子集。
