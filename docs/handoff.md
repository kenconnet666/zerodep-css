...
旧性能脚本和示例已从当前工作树清理，`.research/README.md` 提供历史源码位置，当前原始性能 JSON 与锁文件仍保留。成功测试自动清理自己创建的临时应用；失败诊断保存在忽略的 test-results。此前三个 consumer 临时目录、六个 p4c 注册探针目录，以及本轮八个失败的 Nuxt/Kit 临时应用目录，删除均被自动审批以 blocked by policy 拒绝。用户补充允许清理后，本轮八个目录的重试仍被拒绝；因此保留本机，不绕过限制、不进入 Git。换机不需要复制这些临时文件。
