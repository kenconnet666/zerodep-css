...
export function serviceConfig(kind) {
  if (kind === 'vue')
    return {
      ...
      args: [projectEntry('@vue/language-server', 'bin/vue-language-server.js'), '--stdio'],
      initializationOptions: { typescript: { tsdk: resolve(root, 'node_modules/typescript/lib') } },
    };
  if (kind === 'svelte')
    return {
      bin: process.execPath,
  ...
  return {
    ...
    initializationOptions: {
      ...
      plugins: [
        { name: 'typescript-svelte-plugin', location: root, languages: ['svelte'] },
        {
