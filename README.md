...
这是一个从基础设施重新开始的本地研究分支。当前有 pnpm 工作区、五个 private 包、类型检查和 Codex 语言服务。`core` 已加入少量试验性字符串作者 API、`ic()` 嵌套片段和由宿主提供规则写入的 `createCss()`；**Vue/Svelte 生产适配器与响应式绑定编译器尚未实现，试验 API 尚未定稿**。旧方案可从本地 Git 历史或另存的归档目录查询，不作为本分支实现。
