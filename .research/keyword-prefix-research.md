...
当前生成结果有 502 个属性、12,586 个关键字成员，但不同值只有 718 个。类型提示可以从 csstype 联合类型映射得到，研究类型样板位于 `core/test/types/keyword-shape.ts`，验证了已知成员补全所需的类型、只读约束、排除未知成员和类继承形态。这不是新公开 API，也不代表完整 502 属性的类型替代方案已经迁移完成。
