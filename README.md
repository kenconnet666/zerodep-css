...
这是一个从基础设施重新开始的本地研究分支。当前有 pnpm 工作区、五个 private 包、类型检查和 Codex 语言服务。`core` 已能从固定的 `csstype` 生成首批 10 条属性链与 680 个关键字，并提供 `ic()` 片段和规则去重；Vue/Svelte 已有浏览器上下文、Node 请求宿主及 hydration 的可运行原型。**完整属性生成、Nuxt/SvelteKit 封装和响应式绑定编译器尚未实现，试验 API 尚未定稿**。旧方案可从本地 Git 历史或另存的归档目录查询，不作为本分支实现。
