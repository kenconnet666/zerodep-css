...
本页的 2026-09-23 数据属于历史基线，尚未按新的 createStyles/host API 和作者覆盖语义重跑。当前改造记录见 [production](production.md)，新的双框架运行性能比较安排在 P4；本页不能作为当前提交的速度承诺。

编译期 CSS 主架构的静态、动态值、条件分支、主题与体积对照见 [编译优先研究](compile-first-research.md)。这是新架构研究，尚未修改产品实现。

本页保留历史横向对照与后续同机优化证据，不表示所有性能问题已经解决。不同基准的回调和机器不同，不能直接拼接其数值计算提升倍数。

编译期预计算的后续隔离原型、首次挂载/请求成本、产物体积和落地边界见 [编译期预计算研究](preheat-research.md)。该研究尚未修改生产实现。

## 2026-09-23 静态准备复用与 Builder 按需初始化

本轮以 075b996 为基线落实两项优化，保持公开 API、普通回调逐次执行和框架原生响应式合同。Vue 仅把完全静态且可安全放入 script setup 的准备函数移到组件初始化处；`css` 仍在模板使用点调用。动态单位沿用 computed，Svelte 不增加自己的响应式缓存。Builder 首次访问结构操作时才创建该实例的结构 helper，自定义/原始属性 helper 也按需创建；固定结构名称冲突只验证一次，不缓存业务值或构建实例。

### 同进程计算对照

Node 24.12.0，预热 2,000 次后计时 20,000 次，7 轮交替次序取中位数。普通 runtime 使用 16 组有限值；每次仍执行回调。两个版本的构建结果或完整 snapshot 必须一致。

| 场景   |   Builder 旧→新 | Builder 耗时下降 | 完整 runtime 旧→新 | runtime 耗时下降 |
| ------ | --------------: | ---------------: | -----------------: | ---------------: |
| 单属性 |  67.34→50.82 ms |            24.5% |   136.05→113.17 ms |            16.8% |
| 三属性 | 106.33→88.00 ms |            17.2% |   206.18→185.66 ms |            10.0% |
| 嵌套   | 130.52→99.65 ms |            23.6% |   227.09→198.02 ms |            12.8% |

空 Builder 33.40→18.38 ms。另跑旧版对旧版控制组：非空 Builder 的偏差约 -3.9%～+8.9%，完整 runtime 约 +0.5%～+3.7%。上述阶段独立计时，不能直接相加推导占比。较短的既有运行时/主题探针也保留原始结果，其中部分主题场景在波动范围内，不宣称普遍提速。

### 真实组件对照

Chrome、200 个元素，每版本分别预热；静态场景预热 30 批、计时 200 批、5 轮交替取中位数。父组件无关状态变化，class 仍使用用户原有的静态 `css(s => {...})`；两版均经各自编译插件及官方框架编译器处理，比较完整 manifest、类名和计算样式。

- Vue 静态更新 50.0→27.5 ms，该探针下降 45%；另一次旧版对旧版控制为 81.6→80.9 ms。不同运行的绝对时间不可拼接，也不代表整个应用提速 45%。
- Svelte 静态更新仅 1.4→1.8 ms，控制组同样有微小绝对时间波动。静态 class 本来不会随无关状态重算，本轮不宣称 Svelte 静态收益。
- 普通运行时浏览器对照为 Vue 61.3→57.5 ms、Svelte 65.7→62.8 ms；但控制组本身有约 1%～9% 波动，因此保留为观察值，不视为稳定的页面收益。主题、新规则场景也完整归档，不选择性剔除。

本地通过根 check/build、120 项单元、TS/Vue/Svelte 类型负例与修复清零、真实组件的 SSR/hydration/原生更新和 HMR。新增边界包含隐藏非法静态样式不提前执行、同组件函数复用与跨请求独立、来源映射、未选分支、script 结束标签、静态 HMR 更新，以及延迟 helper 的嵌套所有权和生命周期。其他平台/引擎结果以目标提交 CI 为准。

复现命令：

```sh
node --expose-gc .research/performance/builder-paired.mjs 075b996 repeat
node --expose-gc .research/performance/builder-paired.mjs 075b996 control --control
node .research/performance/framework-paired.mjs 075b996 static --static
node .research/performance/framework-paired.mjs 075b996 static-control --static --control
node .research/performance/framework-paired.mjs 075b996 runtime
```
