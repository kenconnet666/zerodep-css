...
模块选项直接对应可序列化的宿主选项：`namespace`、`layers`、`layer`、`maxRecords`、`warnAt`、`debug`。`compiler` 默认为 `false`；设为 `true` 或 Vue 编译器选项对象才注册可选优化。跨文件项目 hooks 和复杂 CSS 仍保留运行时行为。
...
const global = styles.useGlobalCss('page-background', (g) =>
