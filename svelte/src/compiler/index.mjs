...
export function transformBx(source, filename, options = {}) {
  ...
  function visit(node, restricted = false, locals = new Set()) {
    ...
    if (node.type === 'EachBlock') {
      if (locals.size || !node.key || node.context?.type !== 'Identifier') blocked = true;
      else locals = new Set([...locals, node.context.name, node.index].filter(Boolean));
    }
    const attr = node.attributes?.find((a) => a.type === 'Attribute' && a.name === 'class');
    const value =
      Array.isArray(attr?.value) && attr.value.length === 1 ? attr.value[0] : attr?.value;
    const expression = value?.type === 'ExpressionTag' ? value.expression : undefined;
    if (expression) {
      ...
      if (result.bindings.length) {
        ...
        if (node.attributes.some((a) => source.slice(a.start, a.end).includes('--zbx-')))
    ...
    function inspectExpressions(value) {
      if (!value || typeof value !== 'object') return;
      if (Array.isArray(value)) {
        for (const child of value) inspectExpressions(child);
        return;
      }
      if (value.expression && value.expression !== expression) {
