...
已确认的响应式值在 computed 内读取，避免检查代码提前让父组件订阅这些值。缓存保存规范化后的 class 字符串，因此数组/对象形式也能保持结果稳定；仅 CSS 变量值变化、其他模板内容没有依赖该值时，可更新变量而不重新渲染组件。浅 ref 中的普通对象等不可追踪数据继续回退，保留原求值行为。
