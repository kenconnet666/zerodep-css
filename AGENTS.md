...
- 默认中文沟通。当前已实现 core 样式引擎和 Vue/Svelte 原生响应式、上下文与完整字符串 SSR 接入；ibind 编译尚未实现。实际 API 见各包 README，当前架构见 `.design/architecture.md`，后续计划见 `.design/roadmap.md`，不要把其他候选 API 当作已实现功能。
