...
- 单条可证明的动态单位/raw/token 声明提升为每宿主一次注册的固定变量规则；Vue 组件用 computed、Svelte 组件和 keyed each 行用原生派生值，空值与 CSS-wide 保留直接声明回退。
- 移除未被当前编译产物调用的旧内部值格式器，收窄 `compiler-runtime` 辅助导出；`CssConstruction` 只保留在内部身份桥，Vue/Svelte 根入口的纯类型转出保持一致。
