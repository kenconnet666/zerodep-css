...
- 当前性能入口为 `performance/compare-engines.mjs`、`frameworks.mjs`、`browser-native.mjs`、`profile-runtime.mjs` 与 `cache-paired.mjs`；先根 pnpm build，正式计时串行。其他旧 paired/preheat 脚本与 `api-usability/` 绑定历史 API，不作为当前 checkout 的验证入口。
