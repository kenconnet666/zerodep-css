...
export async function verifyEvidence(browser, output) {
  ...
  await writeFile(
    ...
    JSON.stringify({ runId: 'previous', passed: true }),
  );
  ...
  assert.equal(result.runId, browserRunId);
  const incomplete = JSON.parse(await readFile(resolve(output, 'results.json'), 'utf8'));
  assert.equal(incomplete.passed, false);
  assert.equal(incomplete.runId, browserRunId);
  ...
  await writeFile(
    ...
    JSON.stringify({
      runId: browserRunId,
      status: 'passed',
      note: '主动失败验证截图、trace 和旧成功摘要撤销；results.json 的非成功状态为预期。',
    }) + '\n',
  );
}
